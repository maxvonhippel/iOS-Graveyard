...
- (void)initializeConditionIconLabel
{
    ...
    [self.conditionIconLabel setCenter:CGPointMake(self.container.center.x, 0.29 * self.center.y)];
...
- (void)initializeQuoteLabel
{
    const NSInteger fontSize = 15;
    self.quoteLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.bounds.size.width, 300)];
    //[self.quoteLabel setAdjustsFontSizeToFitWidth:YES];
    [self.quoteLabel setCenter:CGPointMake(self.container.center.x, 0.7 * self.center.y)];
    [self.quoteLabel setFont:[UIFont fontWithName:LIGHT_FONT size:fontSize]];
    [self.quoteLabel setBackgroundColor:[UIColor clearColor]];
    [self.quoteLabel setTextColor:[UIColor whiteColor]];
    [self.quoteLabel setTextAlignment:NSTextAlignmentCenter];
    [self.quoteLabel setLineBreakMode:NSLineBreakByWordWrapping];
    [self.quoteLabel setNumberOfLines:4];
    [self.container addSubview:self.quoteLabel];
